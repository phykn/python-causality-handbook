# Causal Inference for The Brave and True: 한국어 강의노트

인과추론은 어떤 일이 다른 일을 실제로 바꾸었는지 판단하는 방법입니다.

데이터에서 두 일이 함께 보인다고 해서 바로 원인과 결과라고 말할 수는 없습니다. 인과추론은 무엇과 무엇을 비교해야 하는지, 그 비교를 믿으려면 어떤 조건이 필요한지 묻습니다.

이 저장소는 `Causal Inference for The Brave and True`를 참고해 작성하는 한국어 강의노트입니다.

## 목차

1. [상관관계와 인과관계는 왜 다른가](ko/01-correlation-vs-causation.md)
2. [무작위 실험이 강한 이유](ko/02-randomized-experiments.md)
3. [추정값은 왜 흔들리는가](ko/03-why-estimates-vary.md)
4. [숨은 이유가 평균 차이에 섞이는 방식](ko/04-hidden-causes-confounding.md)
5. [선택 편향은 왜 교란요인과 다른가](ko/05-selection-bias.md)
6. [그래프로 원인과 비교를 그려보기](ko/06-causal-graphs.md)
7. [회귀를 조건부 비교로 읽기](ko/07-regression-adjustment.md)
8. [더미 변수와 집단 비교](ko/08-dummy-variables.md)
9. [좋은 통제와 나쁜 통제](ko/09-good-bad-controls.md)
10. [보이지 않는 차이가 남아 있을 때](ko/10-instrumental-variables.md)
11. [쿠폰을 받았다고 모두 사는 것은 아니다](ko/11-non-compliance-and-late.md)
12. [관측 자료에서 비교 대상을 찾는다는 것](ko/12-matching.md)
13. [처치받을 가능성으로 균형 맞추기](ko/13-propensity-score.md)
14. [두 모델을 함께 쓰면 무엇이 나아질까](ko/14-doubly-robust-estimation.md)
15. [전후 변화만 보면 왜 부족할까](ko/15-difference-in-differences.md)
16. [같은 PC방을 여러 번 보면 무엇을 뺄 수 있을까](ko/16-panel-data-fixed-effects.md)
17. [비교할 PC방이 하나도 딱 맞지 않다면](ko/17-synthetic-control.md)

## 출처

- [Causal Inference for The Brave and True](https://matheusfacure.github.io/python-causality-handbook/landing-page.html)
- [matheusfacure/python-causality-handbook](https://github.com/matheusfacure/python-causality-handbook)
