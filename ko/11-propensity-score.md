...
```text
두 사람이 비싼 마우스를 살 가능성이 비슷하다면,
그 가능성을 만든 조건들도 어느 정도 비슷하지 않을까?
```
...
비싼 마우스를 살 것 같은 정도가 비슷한 사람끼리 비교하면, 원래 승률이나 플레이 시간 같은 차이도 조금 덜 섞일 수 있다.
...
이 점수는 사람을 고르기만 위한 점수가 아니라, 비교할 두 사람이 얼마나 비슷한 출발선에 있었는지 보기 위한 점수다.
