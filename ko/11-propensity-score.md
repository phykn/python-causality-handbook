...
조건을 하나하나 맞추는 대신, 먼저 숫자 하나를 만들 수 있다.
...
```text
비슷한 구매 가능성을 가진 사람들 안에서
실제로 비싼 마우스를 산 쪽의 승률이 더 높은가?
```

이 질문이 10장의 매칭과 이어진다.

10장은 조건을 직접 보고 비슷한 사람을 찾았다.

11장은 여러 조건을 사용해 구매 가능성을 계산하고, 그 가능성이 비슷한 사람끼리 비교한다.
...
가운데에서는 이 조건들을 사용해 비싼 마우스를 살 가능성을 계산한다.

오른쪽에서는 성향 점수가 비슷한 사람끼리 비교한다.

## 개인보다 구간으로 보는 편이 쉽다

성향 점수를 처음 볼 때는 개인끼리 완벽하게 비슷하다고 생각하면 헷갈린다.

더 쉬운 방법은 구간으로 보는 것이다.

```text
성향 점수 0.40 ~ 0.50인 사람들
```
...
| --- | --- | ---: |
| 0.40 ~ 0.50 | 비싼 마우스 삼 | 61% |
| 0.40 ~ 0.50 | 비싼 마우스 안 삼 | 57% |
...
구간으로 묶는 대신, 성향 점수가 비슷한 사람끼리 1:1로 붙일 수도 있다.
