...
그래서 이미 설치한 A를 B의 비교 대상으로 쓰면, B의 새 효과와 A의 지난 효과가 함께 섞인다.
...
그래서 그 기간의 전체 변화, 예를 들어 게임 패치나 시즌 변화가 얼마나 있었는지 알려 주는 비교 대상이 된다.

DID가 믿을 만하려면 이 비교 대상이 중요하다.
...
그러면 1주차에서 2주차로 넘어가는 A의 변화를 볼 때, B와 C는 비교 대상으로 쓸 수 있다.
...
비교 대상은 처치를 받지 않은 상태를 대신 보여줘야 한다.

그런데 A는 이미 처치받은 상태다.

그래서 A를 비교 대상으로 쓰면, 비교가 무엇을 말하는지 알기 어려워진다.
...
비교 대상이 이미 처치받은 집단이면 같은 문제가 다시 들어올 수 있다.
