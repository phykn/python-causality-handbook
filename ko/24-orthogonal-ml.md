...
여기서 `남은 새 마우스 여부`라는 말이 어렵다면 이렇게 읽으면 된다.
...
이 생각은 회귀에서도 오래전부터 쓰였다.

여러 조건을 한꺼번에 넣은 회귀를 보더라도, 핵심은 비슷하다.
...
이 결과가 여러 조건을 한 번에 넣은 회귀의 효과와 같아진다는 정리가 있다.

이름은 Frisch-Waugh-Lovell 정리다.

처음부터 이름을 외울 필요는 없다.

이 장에서는 이렇게 기억하면 충분하다.
...
## 핵심은 보조 모델의 실수를 줄이는 것이 아니다

여기서 헷갈리기 쉬운 지점이 있다.

목표는 보조 모델을 완벽하게 만드는 것이 아니다.

승률 예측 모델도 조금 틀릴 수 있다.

새 마우스 받을 가능성 예측 모델도 조금 틀릴 수 있다.

그래도 최종 효과 추정이 그 작은 실수에 너무 크게 흔들리지 않게 만드는 것이 목표다.

방법은 앞에서 본 그대로다.

원래 조건으로 설명되는 큰 차이를 먼저 양쪽에서 뺀다.

그다음 남은 차이끼리 비교한다.

그러면 비교가 원래 실력, 지역, 손님 수 같은 큰 차이에 덜 끌려간다.

이 설계를 **orthogonalization**이라고 부른다.

한국어로 직역하면 직교화지만, 여기서는 이렇게 읽자.

```text
보조 모델의 작은 실수에 덜 흔들리게 비교 구조를 바꾸는 일
```
...
그러면 A의 남은 승률이 이상하게 작아질 수 있다.

왜냐하면 모델이 A를 이미 보고 배웠기 때문이다.
