...
하지만 가끔은 성향 점수를 나중에 맞히지 않아도 된다.

실험이나 캠페인을 설계할 때, 애초에 선택될 확률을 기록해 둔 경우다.

예를 들어 장비 회사가 일부 유저에게 비싼 마우스 할인권을 보냈다고 하자.

이때 회사가 이런 규칙을 미리 정했을 수 있다.
...
그리고 실제로 각 유저가 할인권을 받을 확률을 자료에 저장해 두었다면, 나중에 그 확률을 다시 맞힐 필요가 없다.

그 확률을 그대로 성향 점수로 쓸 수 있다.

이것은 큰 장점이다.

성향 점수를 모델로 맞히면 모델이 틀릴 수 있다.

하지만 배정할 때의 확률을 정확히 기록해 두었다면, 적어도 그 부분은 모델의 실수에 덜 기대게 된다.
